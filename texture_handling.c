...
void	calc_texture_y(t_window *window, int wall_height, int draw_start, int draw_end)
{
	...
	step = 1.0 * window->textures->text_height / (wall_height);
	text_pos = (draw_start - 720 / 2 + wall_height / 2) * step;
	y = draw_start;
	adr = mlx_get_data_addr(window->textures->no, &bpp,
			&l, &endian);
	while (y < draw_end)
	{
...
void	calc_texture(t_window *window)
{
	...
	//printf("%d %d\n", window->textures->text_x, window->textures->wall_x);
...
void	get_texture(t_window *window)
{
	...
	window->textures->we = mlx_xpm_file_to_image(window->mlx,
			...
			&width, &height);
	window->textures->text_width = 64;
	window->textures->text_height = 64;
}
